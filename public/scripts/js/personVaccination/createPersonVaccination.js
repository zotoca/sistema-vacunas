...
window.addEventListener("DOMContentLoaded", async() => {
    let loadInputs = async () => {
        ...
        let doseInput = '<input type="text" name="dose" class="swal2-input" id="person-vaccination-dose"  placeholder="Dosis"/>';

        let lotNumberInput = '<input type="text" name="lot_number" class="swal2-input" id="person-vaccination-lot-number"  placeholder="Numero de lote"/>';

        let dateInput = '<input type="date" name="vaccination_date" class="swal2-input" id="person-vaccination-vaccination-date"  placeholder="Fecha de vacunacion">';

        let isVaccinatedInput = '<label><input type="checkbox" name="is_vaccinated" class="swal2-checkbox" id="person-vaccination-is-vaccinated">¿Esta vacunado?</label>';
        ...
        Swal.fire({
            ...
            preConfirm: async (nothing) => {
                ...
                try {
                    ...
                    const res = await createPersonVaccination(vaccinationId, personId, dose, lotNumber, vaccinationDate, isVaccinated);
                    if (res.message === "ok") {
                        success(
